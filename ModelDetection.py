...
def get_detections_from_image(image: np.ndarray, model: YOLO) -> list[Detection]:
    detections = []
...
def main():
    ...
    cap = cv2.VideoCapture("scene6_front.mp4") # scene 6 is a disaster...
    # torch.hub.help("intel-isl/MiDaS", "DPT_BEiT_L_384", force_reload=True)
    device = "cuda" if torch.cuda.is_available() else "cpu"
    model = YOLO("yolov8n.pt", verbose=False)  # General YOLO Model for Vehicles, Traffic Lights, and Pedestrians
    rand_start = random.randint(600,1000)
    counter = 0
    if not cap.isOpened():
    ...
    while True:
        ...
        if counter < rand_start:
            counter+=1
            continue
        detections = get_detections_from_image(frame, model)
